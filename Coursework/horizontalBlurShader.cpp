...
{
	initShader(L"base_vs.cso", L"horizontal_blur_ps.cso");
}
...
void horizontalBlurShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
	...
	configBufferDesc.ByteWidth = sizeof(HblurBufferType);
...
void horizontalBlurShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& world, const XMMATRIX& view, const XMMATRIX& projection, ID3D11ShaderResourceView* texture, XMFLOAT2 resolution)
{
	...
	// Send config data
	HblurBufferType* configPtr;
	result = deviceContext->Map(configBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	configPtr = (HblurBufferType*)mappedResource.pData;
	configPtr->width = resolution.x;
